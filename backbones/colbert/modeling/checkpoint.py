...
class Checkpoint(ColBERT):
    ...
    def __init__(self, name, colbert_config=None, verbose: int = 3):
        ...
        self.query_tokenizer = QueryTokenizer(name, self.colbert_config, verbose=self.verbose)
        self.doc_tokenizer = DocTokenizer(name, self.colbert_config)
    ...
    def docFromText(
        ...
        if bsize:
            ...
            for input_ids, attention_mask in text_batches:
                batches.append(self.doc(input_ids, attention_mask, keep_dims=keep_dims_, to_cpu=to_cpu))
