...
from backbones.siglip import SiglipVisionModel
...
class PaliGemmaModel(PaliGemmaPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor):
        ...
        selected_image_feature = image_outputs.last_hidden_state # [1, 1024, 1152]
        image_features = self.multi_modal_projector(selected_image_feature)
        image_features = image_features / (self.config.text_config.hidden_size**0.5)
        return image_features # [1, 1024, 2048]

    def get_placeholder_mask_with_delete(
        ...
        # If there are more image tokens than features, truncate the inputs
        if n_image_tokens > n_image_features and input_ids is not None:
            # Find all image token positions
            image_token_positions = torch.nonzero(special_image_mask, as_tuple=False)

            if image_token_positions.size(0) < n_image_features:
                raise ValueError("Not enough image tokens found for the provided features")

            keep_positions = image_token_positions[:n_image_features]
            remove_positions = image_token_positions[n_image_features:]

            keep_mask = torch.ones_like(input_ids, dtype=torch.bool)

            for pos in remove_positions:
                keep_mask[pos[0], pos[1]] = False

            input_ids = input_ids[keep_mask].view(input_ids.shape[0], -1)
            inputs_embeds = inputs_embeds[keep_mask].view(
                inputs_embeds.shape[0], -1, inputs_embeds.size(-1)
            )
        ...
        # 扩展掩码以匹配inputs_embeds维度
        special_image_mask = special_image_mask.unsqueeze(-1).expand_as(inputs_embeds).to(inputs_embeds.device)

        return special_image_mask, input_ids, inputs_embeds
