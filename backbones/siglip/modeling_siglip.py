...
from transformers.utils import ModelOutput, auto_docstring, can_return_tuple, torch_int
    ...
    def forward(self, pixel_values: torch.FloatTensor, interpolate_pos_encoding=False) -> torch.Tensor:
        ...
        if height != self.image_size or width != self.image_size:
            if not interpolate_pos_encoding:
                # 可以选择打印警告信息，提醒用户尺寸发生了变化
                # import warnings
                # warnings.warn(f"Input size ({height}, {width}) doesn't match model's expected size "
                #              f"({self.image_size}, {self.image_size}). Automatically enabling "
                #              f"position encoding interpolation.")
                print("[Debug] interpolate position encoding.")
                interpolate_pos_encoding = True
        # print("pixel_values.shape", pixel_values.shape) # pixel_values.shape torch.Size([4, 3, 448, 448])
        target_dtype = self.patch_embedding.weight.dtype
        patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
        embeddings = patch_embeds.flatten(2).transpose(1, 2)
        # print("embeddings.shape", embeddings.shape) # embeddings.shape torch.Size([4, 1024, 1152])
